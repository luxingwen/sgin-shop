...
type RoleMenuPermission struct {
	...
	CreatedAt time.Time `gorm:"autoCreateTime;type:datetime" json:"created_at"` // CreatedAt 记录了角色菜单权限创建的时间
	UpdatedAt time.Time `gorm:"autoUpdateTime;type:datetime" json:"updated_at"` // UpdatedAt 记录了角色菜单权限信息最后更新的时间
}
